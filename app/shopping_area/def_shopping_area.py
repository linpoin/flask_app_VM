...
def shopping_area_create(name,eg_name,logo,banner,welcome,activity_rule,convert_prize_rule,lottery_method,shop_list,lottery_level_num,repeat_pass,verification_method,prize_list=None):
    ...
    if have_eg_name == None:
      ...
      mysql_engine.execute(f"CREATE DATABASE ShopArea_{eg_name}") #建立database
      ...
      if lottery_method == '0':
        ...
        globals()[f'{eg_name}_engine'].execute('create table prize( id serial not null primary key, prize varchar(80) not null, prize_probability INT(20) not null, all_quantity INT(20) not null, last_quantity INT(20) not null);')
        #植入獎品至獎品資料庫
      ...
      sql_cmd = f"INSERT INTO shopping_area.admin_member (shopping_area_en_name, password) VALUES ('{eg_name}','{hashpassword}')"
      mysql_engine.execute(sql_cmd)
      ...
      for shop_infor in shop_list:
        ...
        myqr.run(words = f'{shop_id}', # 可放網址或文字(限英文)
          picture = shopping_area_data_path + '/image'+f'/{eg_name}.png',
...
def select_shopping_area_info(shop_area_en_name):
  if shop_area_en_name != 'all':
    ...
    if have_en_name != None:
      ...
      #globals()['{}_engine'.format(shop_area_en_name)] = create_engine("mysql+pymysql://{}:{}@{}:3306/ShopArea_{}".format(sql_user,sql_password,sql_ip,shop_area_en_name))
      select_shop_list_q = f"SELECT * FROM shoparea_{shop_area_en_name}.shop;"
      df = pd.read_sql(select_shop_list_q,con=shopping_engine)
      df['shop_qrcode'] = df['shop_qrcode'].map(lambda x: x.decode('utf-8'))
      shop_list = df.to_dict(orient='records')
      #shop_list = shopping_engine.execute(select_shop_list_q).mappings().all()
      rejson = {'shopping_area_name':shopping_area_name,'shopping_area_eg_name':shopping_area_eg_name,'shopping_logo':shopping_logo,'shopping_banner':shopping_banner,'welcome_html':welcome_text,'activity_rule_html':activity_rule_html,'convert_prize_rule_html':convert_prize_rule_html,'activity_rule_text':activity_rule,'convert_prize_rule_text':convert_prize_rule,'lottery_level_num':lottery_level_num,'lottery_method':lottery_method,'shop_list':shop_list}
      return rejson
    else:
...
def select_shopping_area_list():
    ...
    return jsonify(list(map(dict, rejson))),200